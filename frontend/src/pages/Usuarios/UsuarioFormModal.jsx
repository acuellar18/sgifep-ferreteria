...
// Departamentos y roles SIEMPRE se cargan desde la API (fetch real),
// nunca quedan hardcodeados en <option>.
            ...
            {roles.length === 0 ? (
              ...
              roles.map((rol) => (
                <label key={rol.id} className="check-rol">
                  ...
                  />
                  {rol.nombre}
                  {Boolean(rol.acceso_total) && <span className="chip chip-ok">acceso total</span>}
                </label>
              ))
            )}
          </div>
